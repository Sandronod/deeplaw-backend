...
class FetchEuLegislationCommand extends Command
{
    protected $signature = 'eu:fetch-legislation
        {--type=regulation : Document type (regulation|directive|decision)}
        {--limit=100       : Max documents to fetch}
        {--year=           : Filter by year (e.g. 2024)}
        {--chunk=800       : Chunk size in characters}
        {--overlap=100     : Chunk overlap in characters}
        {--dry-run         : Fetch metadata only, no embedding}';

    protected $description = 'Fetch EU legislation from CELLAR SPARQL and embed into eu_documents';
    ...
    public function handle(): int
    {
        ...
        // directives have no dedicated CDM type — filter by title keyword under legislation_secondary
        if ($type === 'directive') {
            $sparql = <<<SPARQL
            ...
            SELECT DISTINCT ?work ?title ?date WHERE {
              ...
              FILTER(REGEX(?title, "^Directive", "i"))
              {$yearFilter}
            }
            ORDER BY DESC(?date)
            LIMIT {$limit}
        ...
        $this->info("Querying CELLAR for {$type}s...");
        ...
        $this->info('Found ' . count($bindings) . ' documents.');
        ...
        $bar->finish();
        $this->newLine();
        $this->info("Done. Stored {$stored} new documents ({$type}).");
    ...
    private function sparqlQuery(string $query): ?array
    {
        try {
            ...
            if (!$response->ok()) {
                Log::error('CELLAR SPARQL error', ['status' => $response->status()]);
                return null;
            }
